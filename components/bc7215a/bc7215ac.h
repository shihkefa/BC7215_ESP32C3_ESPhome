...
public:
    ...
    void                      startCapture();
    // Clear only consumed samples; keep the hardware receiving and UART queued.
    void                      resumeCapture();
    void                      setCaptureProfile(uint8_t count, const bc7215DataMaxPkt_t* data,
...
private:
    BC7215&             bc7215;
    uint8_t expectedCount{0}, expectedStatus[4]{};
    uint16_t expectedBits[4]{};
