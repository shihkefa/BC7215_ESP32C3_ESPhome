...
void BC7215AClimate::received_() {
  ...
  if (ac_->parse(t,m,f,p)) {
    ...
    if (pending_ && desired_.swing_from_remote) {
      desired_.swing = -1;
      desired_.swing_from_remote = false;
    }
    publish_swing_();
    ESP_LOGW(TAG, "IR frame received but not parsed by paired protocol");
    status_message_("收到訊號但無法解析；可試下一個匹配");
  }
...
void BC7215AClimate::loop() {
  ...
  if (ac_->signalCaptured()) {
    ...
    if (ac_->capture_overflow) {
      status_message_("訊號超過4段，本版未處理；請勿長按遙控器");
      ac_->startCapture();
    } else received_();
...
void BC7215AClimate::dump_config() {
  ESP_LOGCONFIG(TAG, "BC7215A ESPHome v1.0.10; library %s", ac_ ? ac_->getLibVer() : "?");
