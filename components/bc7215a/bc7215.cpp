...
// Datasheet reset command: abort the current operation, not an IR transmission.
void BC7215::abortCommand()
{
    if (modPin >= 0 && digitalRead(modPin) == LOW) sendOneByte(0x7a);
}

void BC7215::resetReceiveState()
{
    bc7215Status = {};
    bc7215Status.cmdComplete = 1;
    ...
    curPktInfo = {};
    prePktInfo = {};
    startPos = lastWritingPos = byteCount = 0;
    previousData = 0;
#endif
}
...
void BC7215::sendOneByte(uint8_t data)
{
	...
	if (busyPin != -3)	// if BUSY is connected to arduino
	{
        ...
        while (digitalRead(busyPin) == HIGH) {
            if (millis() - started > 250) { io_error = true; return; }
            delay(1);
